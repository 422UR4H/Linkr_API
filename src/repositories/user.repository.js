...
export async function getAllInfoFromUserIdRefactored(beingLookedId,whoIsLookingId) {
  try {
    const query = `
    SELECT
    json_build_object(
        ...
        'photo', users.photo,
        'following', EXISTS (SELECT 1 FROM followers WHERE follower = $2 AND following = $1),
...
GROUP BY users.id, users.user_name, users.photo;
    ...
    const user = await clientDB.query(query, [beingLookedId, whoIsLookingId]);
